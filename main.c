...
static void USART1_Init(uint32_t baud) {
    ...
    // Calculate USARTDIV
    // For 48MHz PCLK and 115200 baud: USARTDIV = 48,000,000 / (16 * 115,200) ≈ 26.0417
    // Mantissa = 26, Fraction = 0.0417 * 16 ≈ 0.67 → round to 1
    uint32_t pclk = 48000000u;
    uint32_t mantissa = pclk / (16u * baud);
    uint32_t remainder = pclk % (16u * baud);
    uint32_t fraction = (remainder * 16u + (16u * baud) / 2u) / (16u * baud);
...
static inline void USART_Flush(void) {
    while (USART1_STATR & (1u << 5)) {
        (void)USART1_DATAR; // Read and discard
    }
}
...
static void generate_ducoid(void) {
    ...
    uint8_t uid8[8];
    uid8[0] = (uint8_t)(uid_low & 0xFF);
    uid8[1] = (uint8_t)((uid_low >> 8) & 0xFF);
    uid8[2] = (uint8_t)((uid_low >> 16) & 0xFF);
    uid8[3] = (uint8_t)(uid_low >> 24);
    uid8[4] = (uint8_t)(uid_high & 0xFF);
    uid8[5] = (uint8_t)((uid_high >> 8) & 0xFF);
...
static inline uint32_t hex_nibble(char c) {
    return (c >= 'a' && c <= 'f') ? (uint32_t)(c - 'a' + 10) : (uint32_t)(c - '0');
}
...
static const uint32_t kLengthWordByNonceLen[6] = {
    0x00000000u, // 0 (not used)
    0x00000148u, // (40+1)*8 = 328 = 0x148
    0x00000150u, // (40+2)*8 = 336 = 0x150
    0x00000158u, // (40+3)*8 = 344 = 0x158
    0x00000160u, // (40+4)*8 = 352 = 0x160
    0x00000168u  // (40+5)*8 = 360 = 0x168
};
...
int main(void) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (c == ',') break;
            if (c >= '0' && c <= '9') {
                difficulty = difficulty * 10 + (uint32_t)(c - '0');
            } else {
                USART_Flush();
                difficulty = 0;
                break;
        ...
        // Use rdcycle for precise timing measurement
